...
class ItemEncoder(nn.Module):
    ...
    def _build_encoders(self, model_name):
        if "clip" in model_name.lower():
            self.image_enc = CLIPImageEncoder(model_name_or_path=model_name)
            self.text_enc = CLIPTextEncoder(model_name_or_path=model_name)
            self.enc_dim_per_modality = 512  # Override since CLIP uses fixed dim
        else:
